...
#     cfm = Cifar10DataModule('dataset',batch_size=batch_size, download=True)
#     model = RegNet(
...
#         filename="tune_last_ckpt.ckpt", on="validation_end"
#     )

#     trainer = pl.Trainer(
#         max_epochs=num_epochs, accelerator="gpu", logger=logger,
...
if __name__  == "__main__":
    ...
    # if args.tune:
    #     TunePBT(
    #         train_regnet, 'regnet', num_samples=1, num_epochs=30,
    #         cpus_per_trial=24, gpus_per_trial=1
    #     )
    final= RegNet(
        regulated_block=rnn_regulated_block, in_dim=3, h_dim=16, intermediate_channels=32, cell_type='lstm', layers=[3,4,7,4,5]
    )
    ...
    print(x5.shape)
    # x5_array=x5.detach().numpy()
    # combined_image_x5 = x5_array[0].sum(axis=0)
    # plt.imsave('combined_image_x5.png', combined_image_x5, cmap='viridis', format='png')
    ...
    # c_array=c.detach().numpy()
    # combined_image_c = c_array[0].sum(axis=0)
    # plt.imsave('combined_image_c.png', combined_image_c, cmap='viridis', format='png')
    
    # h_array=h.detach().numpy()
    # combined_image_h = h_array[0].sum(axis=0)
    # plt.imsave('combined_image_h.png', combined_image_h, cmap='viridis', format='png')
